package com.example.vedicjyotish.data.network

import com.example.vedicjyotish.data.models.*
...
class VedicApiService(baseUrl: String, private val apiKey: String? = null) {
    ...
    // ──────────────────────────────────────────────────────
    //  POST /api/v1/match/compatibility
    // ──────────────────────────────────────────────────────
    suspend fun calculateCompatibility(
        boy: BirthDataRequest,
        girl: BirthDataRequest,
        method: String = "North"
    ): CompatibilityResponse = withContext(Dispatchers.IO) {
        val payload = mapOf(
            "boy" to boy,
            "girl" to girl,
            "method" to method
        ...
        val json = gson.toJson(payload)
        val request = Request.Builder()
            .url("$baseUrl/api/v1/match/compatibility")
            .post(json.toRequestBody(jsonMediaType))
            .build()
        val response = client.newCall(request).await()
        if (!response.isSuccessful) {
        ...
        val body = response.body?.string() ?: throw ApiException(500, "Empty response")
        gson.fromJson(body, CompatibilityResponse::class.java)
    }
